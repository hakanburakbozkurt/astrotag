...
export function EnergyRulesPopup({ open, onClose }: EnergyRulesPopupProps) {
  return (
    <AnimatePresence>
      {open ? (
        <>
          <motion.button
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] cursor-default bg-black/60 backdrop-blur-md"
            onClick={onClose}
          />
          <motion.div
            role="dialog"
            ...
            className="fixed left-1/2 top-1/2 z-[70] flex w-[calc(100%-1.5rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-xl border border-white/10 bg-[#0a0f1a]/95 shadow-[0_32px_64px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.08)] backdrop-blur-2xl"
