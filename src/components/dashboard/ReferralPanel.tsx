...
export default function ReferralPanel() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      setFriendCode("");
      setMessage(
        `Referans kodu uygulandı! Her ikiniz de +${REFERRAL_STAR_POINTS_BONUS} bonus yıldız kazandınız.`
      );
    } catch (err) {
      setError(
        err instanceof SupabaseActionError
        ...
        <div className="mt-4 space-y-4">
          ...
          {info.hasUsedReferral ? (
            ...
            <form onSubmit={(event) => void handleSubmit(event)} className="space-y-3">
              ...
              >
                {isSubmitting ? "Uygulanıyor..." : `Kodu Kullan (+${REFERRAL_STAR_POINTS_BONUS} Bonus)`}
              </button>
            </form>
          )}
