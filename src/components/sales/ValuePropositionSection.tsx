            ...
            return (
              <li key={step.id}>
                <SalesMotion transition={{ delay: index * 0.06 }}>
                  <Link
                    href={step.href}
                    className="flex h-full flex-col rounded-2xl border border-white/10 bg-gradient-to-b from-[#0f172a]/70 to-[#09090b]/80 p-5 shadow-lg backdrop-blur-xl transition hover:border-zinc-700"
                  >
                    <span className="mb-4 inline-flex h-11 w-11 items-center justify-center rounded-full border border-zinc-700 bg-zinc-900 font-mono text-xs font-bold text-stone-300">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <Icon className="mb-3 h-6 w-6 text-stone-300" aria-hidden />
                    <h3 className="text-base font-semibold text-white">{step.title}</h3>
                    <p className="mt-2 flex-1 text-sm leading-relaxed text-white/50">
                      {step.description}
                    </p>
                    <span className="mt-4 text-xs font-semibold uppercase tracking-[0.14em] text-stone-300">
                      Keşfet →
                    </span>
                  </Link>
                </SalesMotion>
