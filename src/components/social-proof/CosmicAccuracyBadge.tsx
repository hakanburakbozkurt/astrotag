...
function BadgeContent({
    ...
    return (
      <div className="inline-flex flex-col items-center gap-1 sm:items-start">
        <div className="inline-flex items-center gap-2">
          <span className="text-lg font-bold tabular-nums text-stone-300 sm:text-xl">
            {ratingLabel}
          </span>
          <Star
            className="h-4 w-4 fill-stone-300 text-stone-300 drop-shadow-[0_0_10px_rgba(255,255,255,0.08)] sm:h-5 sm:w-5"
            aria-hidden
          />
  ...
  return (
    <div className="inline-flex items-center gap-1.5 text-sm">
      <span className="font-semibold tabular-nums text-stone-300">{ratingLabel}</span>
      <Star
        className="h-3.5 w-3.5 fill-stone-300 text-stone-300 drop-shadow-[0_0_8px_rgba(255,255,255,0.08)]"
        aria-hidden
      />
      <span className="text-stone-300">(%{accuracyLabel} İsabet)</span>
    </div>
  );
}
...
export default function CosmicAccuracyBadge({
  ...
  return (
    ...
    >
      <div
        className={`relative overflow-hidden rounded-full border border-zinc-700 bg-gradient-to-r from-zinc-800 via-zinc-800 to-zinc-900 ${
          isHeader ? "px-4 py-2.5 sm:px-5" : "px-3 py-1.5"
        }`}
        style={{
          boxShadow:
            "0 0 24px rgba(255,255,255,0.08), inset 0 1px 0 rgba(255,255,255,0.06)",
        }}
      >
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse at 30% 50%, rgba(255,255,255,0.08) 0%, transparent 65%)",
          }}
        />
