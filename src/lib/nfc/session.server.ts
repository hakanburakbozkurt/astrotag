...
import {
  clearNfcSessionCookieBundle,
  setNfcSessionCookieBundle,
} from "@/lib/nfc/cookie-session.server";
...
function sessionExpiresAt(): Date {
  return new Date(Date.now() + NFC_SESSION_TTL_MS);
}
...
export async function setNfcSession(params: {
  ...
  const expiresAt = sessionExpiresAt();
  const profileReady = await isProfileComplete(supabase, params.profileId);

  try {
    await setNfcSessionCookieBundle({
      sessionId,
      profileId: params.profileId,
    ...
    const { setLastLoginAtCookie } = await import("@/lib/nfc/auth-persistence.server");
    await setLastLoginAtCookie(expiresAt);
  } catch (error) {
    console.error(
      "[NFC_AUTH_DEBUG]: Hata sebebi setNfcSessionCookieBundle — setNfcSession akışında çerez hatası",
