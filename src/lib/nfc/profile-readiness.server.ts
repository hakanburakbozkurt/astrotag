...
/**
 * PIN başarılı → kayıtlı hedef veya dashboard (profil durumuna göre zorlama yok).
 * @deprecated resolvePostPinLoginRedirect kullanın
 */
export async function resolveRedirectAfterPinLogin(
  _supabase: SupabaseClient,
  _profileId: string,
  _options?: PinLoginRedirectOptions
): Promise<string> {
  const { resolvePostPinLoginRedirect } = await import(
    "@/lib/nfc/post-pin-redirect.server"
  );
  return resolvePostPinLoginRedirect();
}

export type PinLoginRedirectOptions = {
