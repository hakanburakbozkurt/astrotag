...
/**
 * Emph Processing Engine — deterministik astroloji SSOT katmanı.
 * Ephemeris: astronomy-engine (tek kaynak).
 * JSON paketleri Kie/Oracle narration katmanına gider; narration guardrail:
 * @see src/lib/ai/oracle-guardrails.ts
 */
...
import type { EmphComplexity } from "@/lib/cosmic-profile/types";
...
export type EmphReadingMode = "tarot" | "horary" | "cosmic-profile";
...
export function detectCosmicTensions(
...
const COMPLEXITY_LIMITS: Record<
  EmphComplexity,
  { tensions: number; transits: number; includeSynastry: boolean; seeds: number }
> = {
  entry: { tensions: 2, transits: 3, includeSynastry: false, seeds: 3 },
  depth: { tensions: 4, transits: 8, includeSynastry: true, seeds: 5 },
  master: { tensions: 6, transits: 12, includeSynastry: true, seeds: 8 },
};

function applyComplexityToNatal(
  natal: NatalChartSummary,
  complexity: EmphComplexity
): NatalChartSummary {
  if (complexity === "master") {
    return natal;
  }

  const aspectLimit = complexity === "entry" ? 6 : 12;
  return {
    ...natal,
    aspects: natal.aspects.slice(0, aspectLimit),
  };
}

function buildCosmicProfileNarrativeSeeds(
  subjectName: string,
  complexity: EmphComplexity,
  tensions: EmphCosmicTension[],
  transits: CosmicAnalysisContext["transits"]["aspectsToNatal"]
): string[] {
  const limits = COMPLEXITY_LIMITS[complexity];
  const seeds = [
    `Kozmik profil: ${subjectName}`,
    `Seviye: ${complexity}`,
    `Emph motoru natal harita + anlık gökyüzünü ${complexity} derinliğinde kıyasladı.`,
  ];

  for (const tension of tensions.slice(0, limits.seeds - 2)) {
    seeds.push(`Gerilim: ${tension.technicalNote}`);
  }
  ...
  return seeds.slice(0, limits.seeds);
}
...
export async function processCosmicProfileThroughEmph(
  ...
  const { context, natal } = await buildEmphBase(userData, askedAt);
  const limits = COMPLEXITY_LIMITS[complexity];
  const allTensions = detectCosmicTensions(context);
  const cosmicTensions = allTensions.slice(0, limits.tensions);
  const transitsToNatal = context.transits.aspectsToNatal.slice(0, limits.transits);
  const filteredNatal = applyComplexityToNatal(natal, complexity);
