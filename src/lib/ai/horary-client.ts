...
import type { AIHoraryResponse } from "@/lib/ai/horary";

export async function fetchHoraryReading(
  question: string,
  userData: UserData
): Promise<AIHoraryResponse & { cosmicContext?: CosmicAnalysisContext }> {
  ...
  const data = (await response.json()) as AIHoraryResponse & {
    error?: string;
