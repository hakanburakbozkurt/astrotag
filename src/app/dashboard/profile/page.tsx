...
export default function ProfileTabPage() {
  ...
  if (profileLoading || !userData) {
    return (
      <div className="relative flex flex-1 items-center justify-center px-4 py-16">
        <p className="text-sm text-white/45">Profil yükleniyor...</p>
      </div>
    );
  }
  ...
  return (
    <TabPageScaffold
      eyebrow="Profile"
      title={userData.name}
      description="Partner bilgileriniz, hesap güvenliği ve profil yönetimi."
    >
      <motion.section
      ...
      >
        ...
        <h2 className="mt-2 text-lg font-semibold text-white">Synastry Profili</h2>
        <p className="mt-2 text-sm text-white/45">
          {hasExistingPartner
            ? "Kayıtlı partner bilgileriniz aşağıda. Düzenleyip kaydedebilirsiniz."
            : "Partner doğum bilgileri Cosmic Bonds uyumluluk analizini aktive eder."}
        </p>
      ...
      <section className="rounded-[28px] border border-white/10 bg-[#0f172a]/80 p-5 backdrop-blur-2xl sm:p-6">
        ...
        <h2 className="mt-2 text-lg font-semibold text-white">E-posta Giriş Şifresi</h2>
        <p className="mt-2 text-sm text-white/45">
          E-posta ile kayıt olduysanız hesap şifrenizi buradan güncelleyebilirsiniz.
        </p>
        <div className="mt-5">
          <UpdatePasswordForm />
        </div>
